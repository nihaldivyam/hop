...
func (s *Store) CreatePaste(ctx context.Context, p *Paste) error {
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO pastes (id, title, lang, content, size, created_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		p.ID, p.Title, p.Lang, p.Content, p.Size, p.CreatedAt.Unix(), toUnix(p.ExpiresAt))
	if isSQLiteUnique(err) {
		return errExists
	}
	return err
...
func (s *Store) GetPaste(ctx context.Context, id string) (*Paste, error) {
	...
	err := s.db.QueryRowContext(ctx,
		`SELECT id, title, lang, content, size, created_at, expires_at FROM pastes WHERE id = ?`, id).
		Scan(&p.ID, &p.Title, &p.Lang, &p.Content, &p.Size, &created, &exp)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, errNotFound
	}
	if err != nil {
...
func (s *Store) ListPastes(ctx context.Context) ([]Paste, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, title, lang, size, created_at, expires_at FROM pastes
		 WHERE expires_at IS NULL OR expires_at > ? ORDER BY created_at DESC LIMIT 1000`, time.Now().Unix())
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		var created int64
		var exp sql.NullInt64
		if err := rows.Scan(&p.ID, &p.Title, &p.Lang, &p.Size, &created, &exp); err != nil {
			return nil, err
		}
		p.CreatedAt = time.Unix(created, 0).UTC()
		p.ExpiresAt = fromUnix(exp)
